    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('viagems.update', $viagem) }}" method="POST" class="space-y-4">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="data_hora_saida" class="block mt-1 w-full" type="datetime-local" name="data_hora_saida" :value="old('data_hora_saida', $viagem->data_hora_saida ? $viagem->data_hora_saida->format('Y-m-d\TH:i') : '')" required />
                            <x-input-error :messages="$errors->get('data_hora_saida')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="data_hora_chegada" :value="__('Data / Hora de Chegada')" />
                            <x-text-input id="data_hora_chegada" class="block mt-1 w-full" type="datetime-local" name="data_hora_chegada" :value="old('data_hora_chegada', $viagem->data_hora_chegada ? $viagem->data_hora_chegada->format('Y-m-d\TH:i') : '')" />
                    ...
                    <div>
                        <label for="ativo" class="inline-flex items-center">
                            <input id="ativo" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" name="ativo" value="1" {{ old('ativo', $viagem->ativo) ? 'checked' : '' }}>
