    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <form action="{{ route('motoristas.store') }}" method="POST" class="p-6 space-y-5">
            ...
            <div class="relative" id="usuario-autocomplete-wrapper">
                ...
                <!-- Input Oculto enviado ao backend (guarda o ID do usuário selecionado) -->
                ...
                <div class="relative">
                    ...
                    <!-- Campo oculto para persistir o nome exibido caso haja erro de validação -->
                    <input type="hidden" name="usuario_nome" id="usuario_nome_hidden" value="{{ old('usuario_nome') }}">

                    <!-- Botão para Limpar Seleção -->
                    <button type="button" id="btn-clear-usuario" class="{{ old('usuario_id') || old('usuario_nome') ? '' : 'hidden' }} absolute inset-y-0 right-0 flex items-center pr-3 text-slate-400 hover:text-slate-600">
                ...
                <!-- Box de Resultados do Autocomplete -->
...
<!-- Script Autocomplete em Vanilla JS -->
<script>
document.addEventListener('DOMContentLoaded', function () {
    ...
    searchInput.addEventListener('input', function () {
        ...
        debounceTimer = setTimeout(() => {
            fetch(`{{ route('usuarios.autocomplete') }}?q=${encodeURIComponent(query)}`)
